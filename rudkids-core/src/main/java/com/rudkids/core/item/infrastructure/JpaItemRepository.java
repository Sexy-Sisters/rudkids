...
public interface JpaItemRepository extends JpaRepository<Item, UUID> {
    ...
    @Query("SELECT i FROM Item i WHERE i.mystery = false ORDER BY CASE WHEN i.itemStatus = com.rudkids.core.item.domain.ItemStatus.SOLD_OUT THEN 1 ELSE 0 END, i.quantity.value ASC")
