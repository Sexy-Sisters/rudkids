...
public interface JpaItemRepository extends JpaRepository<Item, UUID> {
    ...
    Page<Item> findByMysteryProduct(MysteryProduct mysteryProduct, Pageable pageable);

    @Query("SELECT i FROM Item i WHERE i.mysteryProduct is null ORDER BY CASE WHEN i.itemStatus = com.rudkids.core.item.domain.ItemStatus.SOLD_OUT THEN 1 ELSE 0 END, i.quantity.value ASC")
